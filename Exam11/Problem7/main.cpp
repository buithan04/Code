...
bool CheckLeft (Tree T, int k)
{
    if (T != NULL)
    {
        if (T->info >= k) return false;
        return (CheckLeft(T->left,k) && CheckLeft(T->right,k));
    }
    return true;
}
bool CheckRight (Tree T, int k)
{
    if (T != NULL)
    {
        if (T->info <= k) return false;
        return (CheckRight(T->left,k) && CheckRight(T->right,k));
    }
    return true;
}
bool isBST (Tree T)
{
    if (T != NULL)
    {
        if (!(CheckLeft(T->left,T->info) && CheckRight(T->right,T->info))) return false;
        else
        {
            return (isBST(T->right) && isBST(T->right));
        }
    }
    return true;
}
